...
class OpenAIProvider(BaseModel):
    ...
    def _get_client(self):
        try:
            ...
            raise ImportError("OpenAI provider requires `openai` package. Install with: pip install 'chainforge[openai]'")
        return AsyncOpenAI(
            ...
            for tc in msg.tool_calls:
                ...
                if isinstance(args, str):
                    try:
                        ...
                        args = {"_raw": args}
                tool_calls.append({"id": tc.id, "type": "function", "function": {"name": tc.function.name, "arguments": args}})
        usage = None
        if raw.usage:
    ...
    async def generate(self, messages: list[Message], tools: list[ToolSpec] | None = None, **kwargs: Any) -> LLMResponse:
        ...
        log_data(logger, DEBUG, f"OpenAI generate({self.model})", data={
            "model": self.model, "messages": len(openai_messages), "tools": len(openai_tools) if openai_tools else 0,
            "last_msg": (openai_messages[-1]["content"] or "")[:200] if openai_messages else "",
        })

        try:
            raw = await client.chat.completions.create(
                model=self.model, messages=openai_messages, tools=openai_tools or None, **kwargs,
            )
        except Exception as e:
            log_data(logger, WARNING, f"OpenAI API error: {e}", data={"model": self.model, "error": str(e)})
            raise ProviderError(f"OpenAI API error: {e}") from e

        result = self._parse_response(raw)
        log_data(logger, DEBUG, f"OpenAI response: {result.finish_reason}", data={
            "finish_reason": result.finish_reason, "content_length": len(result.content or ""),
            "tool_calls": len(result.tool_calls) if result.tool_calls else 0,
            "usage": result.usage,
        })
        return result

    async def stream_generate(self, messages: list[Message], tools: list[ToolSpec] | None = None, **kwargs: Any) -> AsyncIterator[str | LLMResponse]:
        client = self._get_client()
        openai_messages = self._to_openai_messages(messages)
        openai_tools = self._to_tool_specs(tools) if tools else None
        log_data(logger, DEBUG, f"OpenAI stream_generate({self.model})", data={"model": self.model, "messages": len(openai_messages)})

        try:
            stream = await client.chat.completions.create(
                model=self.model, messages=openai_messages, tools=openai_tools or None,
            ...
            if finish:
                ...
                if tool_call_deltas:
                    ...
                    for idx in sorted(tool_call_deltas):
                        ...
                        tc_list.append({"id": d["id"], "type": "function", "function": {"name": d["function"]["name"], "arguments": args}})
